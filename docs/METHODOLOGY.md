...
| **B32** | **`suggest_queries` viewpoint templates are English-only — useless for non-English topics.** Live JP run ("日本 新興 ガジェット メーカー") produced lateral angles like `… criticism`, `… alternatives`, `… problems limitations`, `… vs` — English words appended to a Japanese query, which won't surface Japanese criticism/comparison content (the equivalents are 批判/問題点/デメリット/比較/代替). The echo-chamber breaker silently no-ops on non-English research. Fix: detect topic script (CJK presence, or use `region`) and localize the template words (parallel to B29's domain-adaptivity). | suggest_queries actually breaks echo chambers for JP/non-EN research, not just English | — (JP gadget-maker live run, 2026-06-01) disc:2026-06-01 |
| **B33** | **CJK bigram clustering (B22/B28) may over-link on low-entropy bigrams.** Same JP run clustered a gizmodo year-end *roundup* with a Matech *brand explainer* (`story_cluster=1`) — plausibly a false link from coincidental shared CJK bigrams (bigrams carry less signal than Latin words, so `_STORY_MIN_SHARED=2` is looser for CJK). Low-harm (corroboration signal, only 2 items), but consider a higher shared-token threshold or min token length for CJK-dominant titles. | story_cluster precision holds for Japanese/Chinese, not just Latin | — (JP gadget-maker live run, 2026-06-01; low confidence, watch-item) disc:2026-06-01 |
