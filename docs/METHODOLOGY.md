...
| ~~**B37**~~ | ~~B29/B36 primary-source detection misses economics/finance/central-banking~~ | — | **✅ DONE 2026-06-01** disc:2026-06-01 — added `_FINANCE_SIGNALS` (federal/fed/fomc/monetary/inflation/gdp/fiscal/treasury/ecb/…) routing finance topics to the existing OFFICIAL `.gov` template (federalreserve.gov is `.gov`). Only distinctive words (no "rate"/"interest"/"bank" — too generic; verified no misfire on "heart rate"/"API rate limiting"). Bare-entity topics ("Bank of Japan") still default — same B36 limit. **No new lesson** (DoD note): 3rd instance of the already-extracted B25/B36 pattern (domain lists grow; keyword detection's bare-entity blind spot). Pinned by `TestB37FinancePrimarySource`. |
