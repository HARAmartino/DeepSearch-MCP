...
| **B28** | **`story_cluster` (B19) collapses a single-topic search into ONE mega-cluster.** Live "EU AI Act enforcement 2026" run: all 8 results landed in `story_cluster=1` because they all share the *query/topic* tokens (`act`,`2026`,`enforcement`) — which is useless (can't distinguish independent stories). This is exactly the homogeneous-set failure the B19 lesson flagged for live re-check. Fix: exclude the **search query's own tokens** from clustering, and/or down-weight tokens that are ubiquitous across the result set (DF rareness). | story_cluster stays meaningful on the common case (a topic search) instead of grouping everything | — (EU AI Act live run, 2026-06-01) disc:2026-06-01 |
| **B29** | **`suggest_queries` primary-source templates are dev-centric (`site:github`/`site:arxiv`), useless for policy/legal/news topics.** The EU AI Act run found **0 authoritative** sources (all SEO compliance blogs) and `suggest_queries` offered no way to reach the actual primary source (`eur-lex.europa.eu`/`europa.eu`). Add domain-adaptive primary-source angles (official `.gov`/`.europa.eu`/standards bodies) and/or, when a result set is 0-authoritative, nudge a `site:`-official query. | The echo-chamber breaker reaches primary sources for non-dev research too | — (EU AI Act live run, 2026-06-01) disc:2026-06-01 |
...
| Monthly | Re-read this file. Pick one Backlog item and start a cycle. Check `status.py` MTTI: an open flag aging past ~1 week is a prioritization smell. | Maintainer |
