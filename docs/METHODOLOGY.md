...
| **B37** | **B29/B36 primary-source detection misses economics/finance/central-banking.** A "Federal Reserve interest rate decision" run got primary-source angle `site:github.com` — wrong; the primary is `federalreserve.gov` (a `.gov`, i.e. the existing OFFICIAL template would serve it). Add finance/econ signals (federal reserve, interest, rate, inflation, monetary, gdp, central bank, ecb, treasury, markets…) → route to the official/`.gov` template. (B25 list-growth, mirrors B36.) | suggest_queries reaches the central-bank/official primary for finance research | — (Fed-rates live run, 2026-06-01) disc:2026-06-01 |
