...
| **B35** | **0 authoritative is the NORM on real DDG results — and the tool gives no active path to authority.** 4/4 live runs (Rust/EU/JP/medical) returned ~0 `authoritative` results in the top hits; DDG SERPs are dominated by SEO content farms (semaglutide run: telehealthally/peptidedossier/prescriva/… while NEJM/Cochrane/nih.gov never ranked). `source_tier` correctly flags "unknown → corroborate", but offers **no way to reach** the authorities that exist but don't rank. Implement the B29 "0-authoritative nudge": when a result set is ~0-authoritative, surface a domain-appropriate `site:`-authority query (the agent then searches the primary source directly). | Research reaches primary sources even when DDG buries them | — (4 live runs, esp. semaglutide 2026-06-01) disc:2026-06-01 |
| **B36** | **B29 primary-source detection covers only dev + policy — medical/science topics get `site:github`.** The semaglutide run's primary-source angle was `site:github.com` (useless for a drug-safety topic; the real primaries are pubmed/nih.gov/who.int). Extend `_primary_source_template`'s domain detection: medical signals → `site:pubmed.ncbi.nlm.nih.gov OR site:nih.gov OR site:who.int`; science signals → arxiv/`.edu`. (B25 list-growth; also feeds the B35 nudge.) | suggest_queries reaches the right primary source for medical/science research | — (semaglutide live run, 2026-06-01) disc:2026-06-01 |
